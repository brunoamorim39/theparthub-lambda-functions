...
def map_site():
    urls = ['https://www.evomotionshop.com/collections/all-evo-x-parts', 'https://www.evomotionshop.com/collections/evo-vii-ix-parts']
    category_urls = []
    for url in urls:
        main_soup = getSoup(url, headers)
        category_container = main_soup.find('ul', class_='nav-zqobwieu')
        print(category_container)
        categories = category_container.find_all('a', href=True)
        for category in categories:
            cat_url = category['href']
            trimmed_url = '/' + cat_url.split('/')[-1]
            if trimmed_url not in category_urls:
                category_urls.append(trimmed_url)
                print(f'> {trimmed_url}')
    save_to_file_categories(category_urls)

site_name = 'evomotionshop'
