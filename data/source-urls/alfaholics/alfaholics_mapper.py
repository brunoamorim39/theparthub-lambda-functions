...
def find_vehicles():
    ...
    vehicles = main_soup.find_all('a', class_='partLink', href=True)
    for vehicle in vehicles:
        vehicle_href = vehicle['href']
        trimmed_link = f'/{vehicle_href.split("/")[-2]}'
        if trimmed_link not in vehicle_links:
...
def find_categories():
    ...
    for link in links_to_check:
        ...
        cats = vehicle_soup.find_all('a', class_='partLink', href=True)
        for cat in cats:
            cat_href = cat['href']
            trimmed_link = f'/{cat_href.split("/")[-2]}'
            if trimmed_link not in category_links:
                category_links.append(trimmed_link)
                print(f'=> {trimmed_link}')
    save_to_file_categories(category_links)

main_url = f'https://www.alfaholics.com/parts'
site_name = 'alfaholics'
