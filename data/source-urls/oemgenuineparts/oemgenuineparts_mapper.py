...
def find_vehicles():
    urls= ['https://www.oemgenuineparts.com/v-scion', 'https://www.oemgenuineparts.com/v-toyota']
    vehicle_links = []
    for url in urls:
        ...
        for vehicle in vehicles:
            ...
            model_soup = getSoup(f'https://www.oemgenuineparts.com{model_link}', headers)
            try:
                year_container = model_soup.find('div', id='vehicle-data-lists')
                years = year_container.find_all('a', href=True)
                for year in years:
                    year_link = year['href']
                    year_soup = getSoup(f'https://www.oemgenuineparts.com{year_link}', headers)
                    try:
...
def find_categories():
    ...
    with open('vehicles.csv', newline='') as csvfile:
        ...
        for each in csvreader:
            ...
            each_trimmed = each_trimmed.replace("'", '')
            link = f'https://www.oemgenuineparts.com{each_trimmed}'
