...
def find_vehicles():
    url = 'https://www.hondapartsonline.net/v-honda'
    vehicle_links = []
    ...
    for vehicle in vehicles:
        ...
        model_soup = getSoup(f'https://www.hondapartsonline.net{model_link}', headers)
        year_container = model_soup.find('div', id='vehicle-data-lists')
        years = year_container.find_all('a', href=True)
        for year in years:
            year_link = year['href']
            year_soup = getSoup(f'https://www.hondapartsonline.net{year_link}', headers)
            try:
...
def find_categories():
    ...
    with open('vehicles.csv', newline='') as csvfile:
        ...
        for each in csvreader:
            ...
            each_trimmed = each_trimmed.replace("'", '')
            link = f'https://www.hondapartsonline.net{each_trimmed}'
