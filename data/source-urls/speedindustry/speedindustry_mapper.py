...
def save_to_file_vehicles(links):
    with open('vehicles_categories.csv', 'w', newline='') as csvfile:
...
def find_vehicles_and_categories():
    ...
    container = main_soup.find_all('div', class_='entity-body')[-1]
    items = container.find_all('a', href=True)
    for item in items:
        item_link = item['href']
        if item_link not in links:
            links.append(item_link)
            print(f'> {item_link}')
    save_to_file_vehicles(links)

main_url = 'https://speedindustry.com/sitemap'
site_name = 'speedindustry'
